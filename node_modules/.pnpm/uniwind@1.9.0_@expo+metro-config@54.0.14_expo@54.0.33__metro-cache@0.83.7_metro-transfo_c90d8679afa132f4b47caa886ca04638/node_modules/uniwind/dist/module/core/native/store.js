...
const emptyState = { styles: {}, dependencies: [], dependencySum: 0 };
class UniwindStoreBuilder {
  ...
  getStyles(className, componentProps, state, uniwindContext) {
    ...
    const cacheKey = `${className}${state?.isDisabled ?? false}${state?.isFocused ?? false}${state?.isPressed ?? false}${isScopedTheme}${uniwindContext.rtl ?? ""}`;
  ...
  validateDir(rtl, uniwindContext) {
